...
const Header: React.FC<HeaderProps> = ({ activeTab, username, onMenuClick }) => {
  ...
  return (
    <header className="bg-white border-b h-14 flex items-center justify-between px-3 md:px-6 z-10 shadow-sm shrink-0">
      ...
      <div className="flex items-center gap-4">
        <div className="text-right hidden md:block">
          <p className="text-[13px] font-semibold text-slate-800">{username}</p>
          <p className="text-[10px] text-green-600 font-bold uppercase tracking-wider">Online</p>
        </div>
        <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold border border-blue-200 text-sm">
